...
export type TokensResolved = {
  color: Record<string, Record<string, string | null>>;
  spacing: Record<string, Record<string, { value: number } | null>>;
  radius: Record<string, Record<string, { value: number } | null>>;
};

export const tokensResolved: TokensResolved = {
  color: {
...
export function getColorVarName(tokenKey: string): string {
  return `--color-${toVarSegment(tokenKey)}`;
}

export function getSpacingVarName(tokenKey: string): string {
  return `--${toVarSegment(tokenKey)}`;
}

export function getRadiusVarName(tokenKey: string): string {
  return `--${toVarSegment(tokenKey)}`;
}

export function colorBgClass(tokenKey: string): string {
  return `bg-${toVarSegment(tokenKey)}`;
}

export function colorTextClass(tokenKey: string): string {
...
export function getColorHex(
  ...
  if (preferDark && keys.includes("Dark")) return token.Dark;
  if (keys.includes("Light")) return token.Light;
  if (keys.includes("default")) return token.default;
  return token[keys[0]] ?? null;
}
...
export function getSpacingValue(
  token: Record<string, { value: number } | null> | undefined
): number | null {
  if (!token) return null;
  const vals = Object.values(token).filter((v) => v && typeof v.value === "number");
  return vals.length ? (vals[0] as { value: number }).value : null;
}
